...
from tsai.models.utils import Reshape, build_ts_model, build_tabular_model, GAP1d, Noop
from tsai.models.ResCNN import *
...
#        loss_func = BCEWithLogitsLossFlat()
    ...
    #InceptionTimePlus
    ts_model = build_ts_model(ResNetPlus, dls=f.ts_dls
                              ,nf=32
                            #  , c_in = f.ts_dls.vars, c_out=f.ts_dls.c,
                            ,seq_len = f.ts_dls.len 
                            ,fc_dropout=0.3)
                             
    
    tab_model = build_tabular_model(TabModel, dls=f.tab_dls, layers = [200, 100], embed_p = 0.2)


    model = MultiInputNet(ts_model, tab_model) #, c_out =len(f.target) )
    ...
    with mlflow.start_run(run_name=cfg.experiment["run_name"], tags = {"note":cfg.experiment["note"]}) as run:
        ...
        learn.fit_one_cycle(50, lr_max = lr*0.9)
