__all__ = ['ifnone', 'Sequential', 'TabFusionBackbone', 'TabFusionTransformer', 'TSTabFusionTransformer'] # type: ignore

# This is a modified Pytorch implementation based on TabTransformer created by Ignacio Oguiza (oguiza@timeseriesAI.co):
# https://github.com/timeseriesAI/tsai/blob/main/tsai/models/TabFusionTransformer.py
...
from tsai.imports import * # type: ignore
from tsai.utils import *# type: ignore
from tsai.models.layers import * # type: ignore
from tsai.models.utils import * # type: ignore
from tsai.models.positional_encoders import * # type: ignore
from tsai.data.core import *# type: ignore
...
class _TabFusionEncoderLayer(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None, 
        ...
        self.ff = nn.Sequential(nn.Linear(d_model, d_ff), self._get_activation_fn(activation), nn.Linear(d_ff, d_model)) # type: ignore
...
class _TabFusionEncoder(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None, res_dropout=0.1, 
                 activation='gelu', res_attention=False, n_layers=1):
        super().__init__()
...
class TSTabFusionTransformer(nn.Module):
    def __init__(self, c_in, c_out, seq_len, classes, cont_names, 
                 ...
                 fc_mults=(1, .5), fc_dropout=0., fc_act=None, fc_skip=False, fc_bn=False, bn_final=False, init=True, key_padding_mask ='auto'):
    ...
    def forward(self, x):
        ...
        else: x_ts, key_padding_mask = x_ts, None
...
class TSCNNModel(nn.Module):
    def __init__(self, c_in, c_out, seq_len, classes, cont_names, d_model=32, n_layers=6, 
                 kernel_size=3, fc_mults=(1, .5), fc_dropout=0., fc_act=None, 
        ...
        all_dimensions = [mlp_input_size, *hidden_dimensions, c_out]
        self.head_nf = mlp_input_size
        self.head = nn.Sequential(*[_Flatten(), _MLP(all_dimensions, act=fc_act, skip=fc_skip, 
                                                     bn=fc_bn, dropout=fc_dropout, bn_final=bn_final)])
    ...
    def forward(self, x):
        ...
        # Input encoding
        if self.n_emb != 0:
            x_cat = [e(x_cat[:,i]).unsqueeze(2) for i,e in enumerate(self.embeds)]
            x_cat = torch.cat(x_cat, 2)
            x = torch.cat([x, x_cat], 2)
        
        if self.n_cont != 0:
            x_cont = self.conv(x_cont.unsqueeze(1))
            x = torch.cat([x, x_cont], 2)
