...
def create_ranfor(df, cats, conts, dep):
    ...
    rf.fit(trn_xs, trn_y) # type: ignore
    print("mean abs err:",mean_absolute_error(val_y, rf.predict(val_xs))) # type: ignore

    predictions = rf.predict_proba(val_xs)[:,1]
    roc_auc = roc_auc_score(val_y, predictions)# type: ignore
    
    fti = pd.DataFrame(dict(cols=trn_xs.columns, imp=rf.feature_importances_))
    multiplot(val_y, predictions)
    
    rf.val_xs = val_xs # type: ignore
    rf.val_ys = val_y# type: ignore
    rf.trn_xs =trn_xs# type: ignore
    rf.trn_ys = trn_y# type: ignore
