...
from sklearn.metrics import roc_curve, auc, roc_auc_score, brier_score_loss, precision_recall_curve, f1_score, precision_score, recall_score, RocCurveDisplay, PrecisionRecallDisplay
...
def plot_evaluation(y_preds,ys, target):
    ...
    display = RocCurveDisplay(fpr=fpr, tpr=tpr, roc_auc=roc_auc,
                                    estimator_name=f'{target} within 12H post trauma reception').plot(ax=ax[0], c='r')
    
    display2 = PrecisionRecallDisplay.from_predictions(ys, y_preds[:,1], ax=ax[1], figure = fig, name= f'{target} within 12H post trauma reception')#,
                                    #legend=f'{target} within 12H post trauma reception')
    ax[0].plot([0, 1], [0, 1], 'k--', lw=1, c="grey")
    ax[0].set_title("Receiver operator characteristics (ROC)")
    ax[0].grid()
    ax[1].plot([0, 1], [0.5, 0.5], 'k--', lw=1, c="grey")
    ax[1].set_title("Precision Recall (PR)")
    ax[1].grid()
...
def plot_save_fti(fti, threshold=0.0005, save_file=False ):
